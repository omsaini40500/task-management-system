...
let teamCache: Array<{ id: string name: string departmentId?: string }> | null =
  null
...
export async function getDepartments() {
  if (!deptCache)
    deptCache =
      await api.get<Array<{ id: string name: string }>>("/departments")
...
export async function getTeams() {
  if (!teamCache)
    teamCache =
      await api.get<Array<{ id: string name: string departmentId?: string }>>(
        "/teams",
      )
...
export async function getProjects() {
  if (!projCache)
    projCache = await api
      .get<{ items: Array<{ id: string name: string }> }>("/projects")
      .then((res) => res.items)
...
export async function getClients() {
  if (!clientCache)
    clientCache = await api.get<Array<{ id: string name: string }>>("/clients")
...
export async function getUsers() {
  if (!userCache)
    userCache = await api
      .get<{ items: Array<{ id: string name: string avatar: string }> }>(
        "/users",
      )
      .then((res) => res.items)
