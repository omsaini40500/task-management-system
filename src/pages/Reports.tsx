...
export default function Reports() {
  ...
  const fetchSummary = async (selectedPeriod: string) => {
    ...
    try {
      const data = await api.get<ReportSummary>(
        `/reports/summary?period=${selectedPeriod}`,
      )

      setSummary(data)
    } catch (err: any) {
      const message =
        err?.detail || err?.message || "Unable to load report data"
              ...
              <table className="w-full">
                ...
                <tbody>
                  {users.length === 0 ? (
                    ...
                    users.map((u, i) => {
                      const rate = u.tasksTotal
                        ? Math.round((u.tasksCompleted / u.tasksTotal) * 100)
                        : 0
        ...
        return (
          ...
          >
            <div
              ...
              style={{ fontFamily: "DM Sans, sans-serif" }}
            >
              No late task insights are available yet. Update task due dates in
              the backend to display late task summaries.
            </div>
          </motion.div>
        )
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-4">
            ...
            >
              <div
                ...
                style={{ fontFamily: "DM Sans, sans-serif" }}
              >
                Campaign performance is based on current project revenue and
                budget data.
              </div>
            </motion.div>
          </div>
        ...
        return (
          ...
          >
            <div
              ...
              style={{ fontFamily: "DM Sans, sans-serif" }}
            >
              Select a report type or update the period filter to preview
              analytics. Export options will use the current summary data.
            </div>
          </motion.div>
        )
    }
  ...
  const kpis = [
    {
      ...
      change: hasSummary
        ? `${
            summary!.avgProductivityChange > 0 ? "+" : ""
          }${summary!.avgProductivityChange}%`
        : "",
...

    {
      ...
      change: hasSummary
        ? `${
            summary!.tasksCompletedChange > 0 ? "+" : ""
          }${summary!.tasksCompletedChange}%`
        : "",
...

    {
      ...
      change: hasSummary
        ? `${
            summary!.avgHoursPerTaskChange > 0 ? "+" : ""
          }${summary!.avgHoursPerTaskChange}h`
        : "",
...

    {
      ...
      change: hasSummary
        ? `${
            summary!.teamUtilizationChange > 0 ? "+" : ""
          }${summary!.teamUtilizationChange}%`
        : "",
  ...
  return (
    <div className="page">
      ...
      <div className="flex gap-1.5 flex-wrap mb-6">
        {reportTypes.map((r) => (
          <button
            ...
            style={{
              background:
                activeReport === r ? "#6366f1" : "rgba(255,255,255,0.04)",
              ...
              borderColor:
                activeReport === r ? "#6366f1" : "rgba(255,255,255,0.07)",
            }}
          >
            {r}
          </button>
      ...
      {error ? (
        <div
          className="card p-4 mb-6 text-sm text-red-300 flex items-center justify-between"
      ...
      {loading ? (
        <div
          className="card p-6 text-center text-sm text-slate-300"
