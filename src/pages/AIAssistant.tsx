...
function getAiReply(msg: string): string {
  ...
  if (m.includes('performance') || m.includes('campaign')) return aiResponses.performance
...
export default function AIAssistant() {
  ...
  function send(text?: string) {
    ...
    setLoading(true)
    setTimeout(() => {
      setMessages(prev => [...prev, { role: 'ai', content: getAiReply(msg), ts: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }])
      setLoading(false)
    }, 1200)
  }
  ...
  return (
    <div className="h-full flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-bold text-white" style={{ fontFamily: 'DM Sans, sans-serif' }}>AI Assistant</h1>
        <p className="text-sm mt-1" style={{ color: '#64748b' }}>Your intelligent marketing operations co-pilot</p>
      </div>

      <div className="flex gap-4 flex-1 min-h-0">
        <div className="flex-1 flex flex-col rounded-xl border overflow-hidden" style={{ backgroundColor: '#1c2340', borderColor: '#252d4a' }}>
          <div className="flex-1 overflow-auto p-5 space-y-4">
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-sm"
                  style={{ backgroundColor: m.role === 'ai' ? 'rgba(99,102,241,0.2)' : 'rgba(139,92,246,0.2)' }}>
                  {m.role === 'ai' ? '🤖' : '👤'}
                </div>
                <div className={`max-w-lg ${m.role === 'user' ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                  <div className="rounded-xl px-4 py-3 text-sm leading-relaxed whitespace-pre-line"
                    style={{ backgroundColor: m.role === 'ai' ? '#252d4a' : 'rgba(99,102,241,0.2)', color: m.role === 'ai' ? '#e2e8f0' : '#e2e8f0' }}>
                    {m.content}
                  </div>
                  <span className="text-xs" style={{ color: '#64748b' }}>{m.ts}</span>
                </div>
              </div>
            ))}
            {loading && (
              <div className="flex gap-3">
                <div className="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center" style={{ backgroundColor: 'rgba(99,102,241,0.2)' }}>🤖</div>
                <div className="rounded-xl px-4 py-3 flex gap-1 items-center" style={{ backgroundColor: '#252d4a' }}>
                  {[0, 1, 2].map(d => (
                    <div key={d} className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ backgroundColor: '#6366f1', animationDelay: `${d * 150}ms` }} />
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="p-4 border-t" style={{ borderColor: '#252d4a' }}>
            <div className="flex gap-2">
              <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()}
                placeholder="Ask anything about your campaigns, tasks, or team..." className="flex-1 rounded-lg px-4 py-2.5 text-sm border"
                style={{ backgroundColor: '#0d1117', borderColor: '#252d4a', color: '#e2e8f0' }} />
              <button onClick={() => send()} className="px-4 py-2.5 rounded-lg text-sm font-medium text-white" style={{ backgroundColor: '#6366f1' }}>Send</button>
            </div>
          </div>
        </div>
        ...
        <div className="w-60 space-y-4">
          <div className="rounded-xl p-4 border" style={{ backgroundColor: '#1c2340', borderColor: '#252d4a' }}>
            <p className="text-xs font-medium mb-3" style={{ color: '#64748b' }}>QUICK PROMPTS</p>
            {suggestions.length === 0 ? (
              <div className="text-sm text-center" style={{ color: '#94a3b8' }}>Quick prompts are unavailable while AI backend integration is pending.</div>
            ) : (
              <div className="space-y-2">
                {suggestions.map((s) => (
                  <button key={s} onClick={() => send(s)} className="w-full text-left px-3 py-2 rounded-lg text-xs transition-colors"
                    style={{ backgroundColor: '#252d4a', color: '#94a3b8' }}
                    onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = 'rgba(99,102,241,0.15)')}
                    onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = '#252d4a')}>
                    {s}
                  </button>
                ))}
              </div>
          ...
          <div className="rounded-xl p-4 border" style={{ backgroundColor: '#1c2340', borderColor: '#252d4a' }}>
            <p className="text-xs font-medium mb-3" style={{ color: '#64748b' }}>CAPABILITIES</p>
            {['AI Chat', 'Task Generator', 'Meeting Summary', 'Report Generator', 'Productivity Tips', 'Smart Search'].map(c => (
