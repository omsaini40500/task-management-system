...
function getWeeklyTasksData(tasks: Task[]) {
  ...
  DAYS.forEach((d) => {
    counts[d] = { completed: 0, created: 0 }
  })
...
export default function Dashboard() {
    ...
    const load = async () => {
      try {
        ...
        const [
          ...
          api
            .get<{ items: Task[] }>("/tasks/my?limit=25")
            .then((r) => (Array.isArray(r?.items) ? r.items : [])),

          api
            .get<{ items: Project[] }>("/projects?limit=25")
            .then((r) => (Array.isArray(r?.items) ? r.items : [])),
        ...
        setTeamMemberCount(
          Array.isArray(allUsers)
            ? allUsers.filter((u: any) => u.isActive).length
            : 0,
        )
        ...
        Promise.all([
          api
            .get<{ items: Task[] }>("/tasks?skip=25&limit=1000")
            .then((r) => (Array.isArray(r?.items) ? r.items : [])),

          api
            .get<{ items: Task[] }>("/tasks/my?skip=25&limit=1000")
            .then((r) => (Array.isArray(r?.items) ? r.items : [])),

          api
            .get<{ items: Project[] }>("/projects?skip=25&limit=1000")
            .then((r) => (Array.isArray(r?.items) ? r.items : [])),
          ...
          .then(([moreTasks, moreMine, moreProjects, moreExpenses]) => {
            if (moreTasks.length)
              setTasks((prev) => {
                const exist = new Set(prev.map((t) => t.id))
                return [...prev, ...moreTasks.filter((t) => !exist.has(t.id))]
              })

            if (moreMine.length)
              setMyTasks((prev) => {
                const exist = new Set(prev.map((t) => t.id))
                return [...prev, ...moreMine.filter((t) => !exist.has(t.id))]
              })
            ...
            if (moreProjects.length)
              setProjects((prev) => {
                ...
                return [
                  ...prev,
                  ...moreProjects.filter((t) => !exist.has(t.id)),
                ]
              })

            if (moreExpenses.length)
              setExpenses((prev) => {
                const exist = new Set(prev.map((t) => t.id))
                return [
                  ...prev,
                  ...moreExpenses.filter((t) => !exist.has(t.id)),
                ]
              })
          })
          .catch(console.error)
  ...
  const teamPerfData = projects.map((p) => ({
    ...
    productivity:
      p.tasks > 0 ? Math.round((p.completedTasks / p.tasks) * 100) : 0,
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
        {(!isOnlyMember
          ...
          <motion.div
            ...
            className={isOnlyMember ? "col-span-2" : ""}
          >
            <KpiCard {...kpi} />
          </motion.div>
        ))}
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        background: utilization > 90 ? "#ef4444" : kpi.color,
                      }}
                    />
                  </div>
                )}
              ...
              {teamPerfData.length === 0 ? (
                ...
                teamPerfData.map((team, i) => (
                  ...
                  >
                    ...
                    <div className="text-xs font-semibold w-10 text-right text-white">
                      {team.productivity}%
                    </div>
                    <div
          ...
          <div className="space-y-2">
            {myTasks.length === 0 ? (
              ...
              myTasks.slice(0, 6).map((task) => (
                <div
                  key={task.id}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl table-row cursor-pointer"
                >
                  <div
              ...
              {projects
                ...
                .map((proj, i) => (
                  <motion.div
                    ...
                    className="flex items-center gap-3 p-3 rounded-xl table-row cursor-pointer"
                  >
                    <div
                    ...
                    <div className="text-right">
                      <div className="text-xs text-white font-medium">
                        {proj.completedTasks}/{proj.tasks}
                      </div>
