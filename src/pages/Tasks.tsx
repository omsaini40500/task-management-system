        ...
        <div className="mb-3">
          ...
          >
            <motion.div
              ...
              style={{
                background: task.progress === 100 ? "#22c55e" : "#6366f1",
              }}
            />
          </div>
        </div>
    ...
    >
      ...
      <div className="p-6">
        ...
        {(currentStatus === "blocked" || currentStatus === "in_progress") && (
          ...
          >
            <label className="block text-xs font-medium text-red-400 mb-1">
              Reason for{" "}
              {currentStatus === "blocked" ? "Blocker" : "Pending Status"}
            </label>
            <textarea
              rows={2}
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            { label: "Estimated", value: `${task.estimatedDays}d` },
            { label: "Spent", value: `${task.spentHours}h` },
            {
              label: "Remaining",
              value: `${Math.max(0, (task.estimatedDays * 8) - task.spentHours).toFixed(1)}h`,
            },
          ].map((item) => (
            <div
              key={item.label}
        ...
        <div className="mb-5">
          ...
          >
            <div
              ...
              style={{ width: `${task.progress}%`, background: "#6366f1" }}
            />
          </div>
        </div>
...
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([usersData, initialTasks]) => {
        ...
        api
          .get<{ items: Task[] }>("/tasks?skip=15&limit=1000")
          .then((r) => {
  ...
  const handleSendDailyReport = async () => {
    ...
    if (sent) {
      setEmailStatus(
        "Daily task summary email sent to super admin successfully!",
      )
    ...
    <div className="page">
        ...
        <div className="flex items-center gap-2">
          ...
          {emailStatus && (
            <span
              ...
              style={{
                background: emailStatus.includes("success")
          ...
          {statusCols.map((col) => {
            ...
            return (
              <div key={col.key} className="flex-shrink-0 w-72">
                ...
                <div
                  className="space-y-2 min-h-24 rounded-2xl p-2"
                  style={{
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full py-2 rounded-xl text-xs transition-smooth flex items-center justify-center gap-1"
                    style={{ color: "#4b5563" }}
                    onMouseEnter={(e) =>
      ...
      {view === "list" && (
        <div className="card overflow-hidden">
          <div
            className="grid text-xs font-semibold uppercase tracking-widest px-4 py-3 border-b"
            style={{
          ...
          <AnimatePresence>
            {filtered.map((task, i) => (
              ...
              >
                ...
                <span className={`badge priority-${task.priority} text-xs`}>
                  {task.priority}
                </span>
                <span
                  className={`badge status-${task.status.replace("_", "-")} text-xs`}
                >
          ...
          <div className="grid grid-cols-7 gap-2">
            {Array.from({ length: 35 }, (_, i) => {
              ...
              return (
                <div
                  key={i}
                  className="min-h-20 rounded-xl p-2 transition-smooth"
                  style={{
                    background:
                      day < 1 || day > 30
                        ? "transparent"
                  ...
                  {day >= 1 && day <= 30 && (
                    <>
                      <div
                        ...
                        style={{ color: isToday ? "#818cf8" : "#6b7280" }}
                      >
                        {day}
                      </div>
                      {dayTasks.slice(0, 2).map((t) => (
                        <div
                          ...
                          className="text-xs px-1.5 py-0.5 rounded-md mb-0.5 truncate cursor-pointer"
                          style={{
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const payload = {
                        ...
                        estimated_days: Number(formData.estimatedDays) || 0,
